...
### 2.  SQL 활용
  ...
  11. 조인 Join
      ...
      - Join 유형
        ...
        - 논리적 조인
          ...
          - 외부 조인 Outer Join
            - 왼쪽 Left Outer Join : 왼쪽 테이블의 모든 데이터와 오른쪽 테이블의 동일 데이터 추출
  ...
  12. 서브쿼리 Sub-Query
      ...
      - 서브쿼리 유형
        ...
        - 동작 방식 기준
          -  비연관 Un-Correlated Sub-Query
             -  서브쿼리가 메인쿼리의 컬럼을 가지고 있지 않은 형태
             - 메인쿼리에 서브쿼리에서 실행된 결과 제공하는 용도
          - 연관 Correlated Sub-Query
            - 서브쿼리가 메인쿼리의 컬럼에 가지고 있는 형태
        ...
        - 데이터 형태 기준
          ...
          - 다중 행 Multiple Row
            - 실행 결과가 여러 건
            - 다중 행 비교 연산자 (IN, ALL, ANY, EXISTS) 사용
            - IN : 임의의 값과 동일한 조건
            - ALL : 모든 값을 만족하는 조건
